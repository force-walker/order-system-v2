...
export const InvoiceDraftPage = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError('');
      try {
        const data = await listInvoiceDraftListRows();
        setRows(data);
      } catch (e) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows
      ...
      .sort((a, b) => `${a.customerName}${a.productName}`.localeCompare(`${b.customerName}${b.productName}`, 'ja'));
  }, [rows, customerFilter, productFilter, statusFilter]);
    ...
    <section>
      <div className="card">
        <div className="list-header">
          <div>
            <h2>請求ドラフト一覧</h2>
            <p className="subtle">納品確認データ由来の請求ドラフトを確認します。</p>
          </div>
        </div>
        ...
        <div className="list-controls" style={{ marginBottom: 12 }}>
          ...
          <label className="filter-label">
            商品名
            <input value={productFilter} onChange={(e) => setProductFilter(e.target.value)} placeholder="商品名で検索" />
          </label>
          <label className="filter-label">
            ステータス
        ...
        {filtered.length === 0 ? (
          <EmptyState title="対象データがありません" description="条件に合う請求データがありません。" />
        ) : (
